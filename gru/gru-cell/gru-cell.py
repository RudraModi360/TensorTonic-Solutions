...
def gru_cell(x_t: np.ndarray, h_prev: np.ndarray,
    ...
    r_t=sigmoid(combined@W_r.T+b_r)
    z_t=sigmoid(combined@W_z.T+b_z)
    h_tlide=np.tanh(np.concatenate((r_t*h_prev,x_t),axis=-1)@W_h.T+b_h)
    h_t=z_t*h_prev+(1-z_t)*h_tlide
    return h_t